...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps, scale_betas=1.0):
    ...
    elif schedule_name.startswith("cosine_tau_"):
        tau = float(schedule_name.replace("cosine_tau_", ""))
        return betas_for_alpha_bar(
            num_diffusion_timesteps,
            lambda t: math.cos((t + 0.008) / 1.008 * math.pi / 2) ** (2 * tau),
        )
    else:
...
class GaussianDiffusion:
    ...
    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        ...
        return _extract_into_tensor(1.0 / self.posterior_mean_coef1, t, x_t.shape) * xprev - _extract_into_tensor(self.posterior_mean_coef2 / self.posterior_mean_coef1, t, x_t.shape) * x_t  # (xprev - coef2*x_t) / coef1
